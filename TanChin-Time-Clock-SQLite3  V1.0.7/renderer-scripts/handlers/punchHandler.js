...
export async function handlePunch() {
    ...
    if (lastValidPunch && (now.getTime() - lastValidPunch.timestamp) < 60 * 1000) {
        ...
        state.punchRecords.unshift(duplicateRecord);
    ...
    if (result.success) {
        state.punchRecords.unshift(newRecord);
        const greetingList = state.greetings[punchType] || [];
...
export async function executeManualPunch() {
    ...
    if (result.success) {
        state.punchRecords.unshift(newRecord);
        state.punchRecords.sort((a, b) => b.timestamp - a.timestamp);
