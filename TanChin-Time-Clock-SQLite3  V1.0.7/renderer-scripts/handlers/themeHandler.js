...
export function renderThemeSchedules() {
    ...
    const themeDisplayNames = { 'default': '預設主題', 'christmas': '聖誕主題', 'cny': '春節主題' };
...
function getThemeEditorStyles() {
    ...
    return {
        ...
        clockDecoLeft: editor.clockDecoLeft.value,
        clockDecoRight: editor.clockDecoRight.value,
...
export function updateThemeEditorPreview() {
            ...
            <div class="magic-container w-full p-6 rounded-2xl" id="preview-magic-container">
                <header class="text-center mb-6">
                    ...
                    <div id="preview-clock-panel" class="bg-gray-800 text-white rounded-lg p-4 mt-4 text-center shadow-inner relative">
                        <div id="preview-clock-decoration" class="clock-decoration"></div>
    ...
    const preview = {
        ...
        clockDecoration: document.getElementById('preview-clock-decoration'),
    ...
    const decoLeft = styles.clockDecoLeft ? `<div>${styles.clockDecoLeft.repeat(3).split('').join('</div><div>')}</div>` : '';
    const decoRight = styles.clockDecoRight ? `<div>${styles.clockDecoRight.repeat(3).split('').join('</div><div>')}</div>` : '';
    preview.clockDecoration.innerHTML = `<div class="clock-decoration-left">${decoLeft}</div><div class="clock-decoration-right">${decoRight}</div>`;
...
export function populateThemeEditorLoadSelector() {
    ...
    select.innerHTML = `
        ...
        <option value="christmas">聖誕主題</option>
        <option value="cny">春節主題</option>
    `;
...
export function loadThemeForEditing(themeId) {
    ...
    if (themeId === 'default') {
        ...
        themeToLoad = {
            bgStart: '#0250c5', bgEnd: '#d43f8d',
            clockBg: '#1e6b34',
            clockDecoLeft: '❄️', clockDecoRight: '❄️',
            clockSymbolsLeft: '☃️', clockSymbolsRight: '🎄',
            punchEffect: 'emoji', punchEmojis: '❄️,☃️,🎁',
        };
        themeName = '我的聖誕主題';
    } else if (themeId === 'cny') {
        themeToLoad = {
            bgStart: '#b82e1f', bgEnd: '#e34425',
            clockBg: '#a81c07',
            clockDecoLeft: '🏮', clockDecoRight: '🏮',
            clockSymbolsLeft: '🧧', clockSymbolsRight: '🎊',
            punchEffect: 'emoji',
            punchEmojis: '🎉,🎊,🧧',
        };
        themeName = '我的春節主題';
    ...
    Object.keys(themeToLoad).forEach(key => {
        const mapping = {
            ...
            clockDecoLeft: 'theme-editor-clock-deco-left', clockDecoRight: 'theme-editor-clock-deco-right',
        ...
        if (elem) {
            if(elem.type === 'checkbox') elem.checked = themeToLoad[key];
            else elem.value = themeToLoad[key];
        }
    });
...
function applyCustomTheme(styles) {
    ...
    const decoLeft = styles.clockDecoLeft ? `<div>${styles.clockDecoLeft.repeat(3).split('').join('</div><div>')}</div>` : '';
    const decoRight = styles.clockDecoRight ? `<div>${styles.clockDecoRight.repeat(3).split('').join('</div><div>')}</div>` : '';
    ui.clockDecorationContainer.innerHTML = `<div class="clock-decoration"><div class="clock-decoration-left">${decoLeft}</div><div class="clock-decoration-right">${decoRight}</div></div>`;
}
...
export function checkAndApplyThemeSchedule() {
    ...
    if (themeToApply === 'default') {
        // 預設樣式，不做事
    } else if (themeToApply === 'christmas' || themeToApply === 'cny') {
        document.body.classList.add(`theme-${themeToApply}`);
        ui.clockPanel.classList.add(`theme-${themeToApply}`);
    } else {
...
export function triggerPunchEffect() {
    ...
    } else if (activeThemeId === 'christmas') {
        effect = 'emoji';
        emojis = '❄️,☃️,🎁';
    } else if (activeThemeId === 'cny') {
        effect = 'emoji';
        emojis = '🎉,🎊,🧧';
    }
