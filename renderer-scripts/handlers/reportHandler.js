...
export function showReportLogin() {
    ...
    // ✨ 魔法修正：呼叫 showModal 時，告訴它要將焦點放在輸入框上
    showModal('report-login-modal', 'report-login-input');
}
...
export function handleReportDateRange() {
    ...
    // ★ [1] 區塊開始: 修正日期查詢邏輯 ★
    // 原本的 new Date(startDateStr).getTime() 會將日期解析為 UTC 零時，
    // 對於 UTC+8 來說，這會變成當天的早上 8 點。
    // 我們將其修改為 .setHours(0, 0, 0, 0)，以確保查詢是從本地時間的 0 點 0 分開始。
    const startTime = new Date(startDateStr).setHours(0, 0, 0, 0);
