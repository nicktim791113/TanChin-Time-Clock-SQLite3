 ...
 * 渲染問候語列表
 */
export function renderGreetingsList() {
    const listContainer = document.getElementById('greetings-list');
    listContainer.innerHTML = '';

    if (state.greetings.length === 0) {
        listContainer.innerHTML = `<p class="text-gray-400 text-center p-4">還沒有設定任何問候語喔！</p>`;
        return;
    }

    // 將通用與個人化問候語分開並排序
    const generalGreetings = state.greetings.filter(g => !g.employee_id);
    const personalGreetings = state.greetings.filter(g => g.employee_id);

    const renderGroup = (title, greetings) => {
        if (greetings.length > 0) {
            const groupTitle = document.createElement('h4');
            groupTitle.className = 'text-md font-semibold text-gray-500 mt-4 first:mt-0';
            groupTitle.textContent = title;
            listContainer.appendChild(groupTitle);

            greetings.forEach(greeting => {
                const empName = greeting.employee_id ? (state.employees.find(e => e.id === greeting.employee_id)?.name || `工號:${greeting.employee_id}`) : '通用';
                const typeText = greeting.type === 'in' ? '上班' : '下班';
                
                const div = document.createElement('div');
                div.className = 'p-2 bg-white rounded-lg border';
                div.innerHTML = `
                    <div class="flex justify-between items-center">
                        <p class="font-bold text-sm text-gray-800">${empName} - ${typeText}</p>
                        <div class="space-x-2">
                            <button class="text-blue-600 hover:underline text-sm edit-greeting-btn" data-id="${greeting.id}">編輯</button>
                            <button class="text-red-600 hover:underline text-sm delete-greeting-btn" data-id="${greeting.id}">刪除</button>
                        </div>
                    </div>
                    <p class="text-xs text-gray-600 whitespace-pre-wrap">${greeting.message}</p>
                `;
    ...
    renderGroup('通用問候語', generalGreetings);
    renderGroup('個人化問候語', personalGreetings);
}
...
function resetGreetingForm() {
    ...
    modal.querySelector('#greetings-form-title').textContent = '新增問候語';
    modal.querySelector('#save-greeting-btn').textContent = '儲存';
    modal.querySelector('#cancel-greeting-edit-btn').classList.add('hidden');
    modal.querySelector('#greeting-type').value = 'in';
    modal.querySelector('#greeting-message').value = '';
    modal.querySelector('#greeting-emp-id').value = '';
}
...
export async function saveGreeting() {
    const modal = ui.modals.greetings;
    const type = modal.querySelector('#greeting-type').value;
    const message = modal.querySelector('#greeting-message').value.trim();
    const employee_id = modal.querySelector('#greeting-emp-id').value.trim() || null;

    if (!message) {
        showToast('問候語內容不可為空！', 'error');
        return;
    }
    if (employee_id && !state.employees.some(e => e.id === employee_id)) {
        showToast(`找不到工號為 "${employee_id}" 的員工！`, 'error');
        return;
    }

    const greetingData = { type, message, employee_id };
    let updatedGreetings = [...state.greetings];

    if (state.editingGreetingId) {
        const index = updatedGreetings.findIndex(g => g.id === state.editingGreetingId);
        if (index > -1) {
            updatedGreetings[index] = { ...updatedGreetings[index], ...greetingData };
        }
    } else {
        const newGreeting = { id: `greeting_${Date.now()}`, ...greetingData };
        updatedGreetings.push(newGreeting);
    }
 ...
 * @param {Event} event 
 */
export function handleGreetingAction(event) {
    const target = event.target;
    const greetingId = target.dataset.id;
    if (!greetingId) return;

    const greeting = state.greetings.find(g => g.id === greetingId);
    if (!greeting) return;

    if (target.classList.contains('edit-greeting-btn')) {
        setState({ editingGreetingId: greetingId });
        const modal = ui.modals.greetings;
        modal.querySelector('#greetings-form-title').textContent = '編輯問候語';
        modal.querySelector('#save-greeting-btn').textContent = '儲存變更';
        modal.querySelector('#cancel-greeting-edit-btn').classList.remove('hidden');
        modal.querySelector('#greeting-type').value = greeting.type;
        modal.querySelector('#greeting-message').value = greeting.message;
        modal.querySelector('#greeting-emp-id').value = greeting.employee_id || '';
    } else if (target.classList.contains('delete-greeting-btn')) {
        showConfirm('刪除問候語', '您確定要刪除此問候語嗎？', async () => {
            const updatedGreetings = state.greetings.filter(g => g.id !== greetingId);
            const result = await dbRequest('saveGreetings', updatedGreetings);
            if (result.success) {
                setState({ greetings: updatedGreetings });
                renderGreetingsList();
                showToast('問候語已刪除！', 'success');
            } else {
...
export function renderBellHistoryInModal() {
    ...
    const sortedHistory = state.bellHistory.slice().sort((a, b) => b.timestamp - a.timestamp);
    const maxVisible = 500;
    const visibleHistory = sortedHistory.slice(0, maxVisible);

    if (sortedHistory.length > maxVisible) {
        const summary = document.createElement('p');
        summary.className = 'text-xs text-gray-500 mb-2';
        summary.textContent = `目前僅顯示最新 ${maxVisible} 筆響鈴紀錄；完整資料仍可透過匯出取得。`;
...
// ★ [7] 區塊開始: 修改儲存函式，使其能同時處理主標題與副標題
/**
 * 儲存資料設定 (主標題與副標題)
 */
export async function saveDataSettings() {
    const newTitle = ui.mainTitleInput.value.trim();
    const newSubtitle = ui.subTitleInput.value.trim();

    if (!newTitle) {
        showToast('主標題不可為空！', 'error');
        return;
    }

    try {
        // 使用 Promise.all 一次性發送所有儲存請求
        const results = await Promise.all([
            dbRequest('setSetting', 'mainTitle', newTitle),
            dbRequest('setSetting', 'subtitle', newSubtitle)
        ]);
        ...
        if (allSuccess) {
            setState({ mainTitle: newTitle, subtitle: newSubtitle });
            ui.mainTitle.textContent = newTitle;
            ui.subTitle.textContent = newSubtitle;
            showToast('資料設定已更新！', 'success');
        } else {
            // 如果有任何一個請求失敗，就找出錯誤訊息
            const firstError = results.find(r => !r.success).error || '未知錯誤';
...
export function openChangePasswordModal(type) {
    ...
    if (type === 'admin') {
        ...
        currentPassLabel.textContent = '請輸入「系統密碼」以授權變更';
    } else { // system
    ...
    try {
        ...
        if (state.currentPasswordChangeType === 'admin') {
            if (currentPass !== state.systemPassword) {
                showToast('系統密碼錯誤！您需要最高權限才能變更管理者密碼。', 'error');
                return;
            }
