...
function setupElectronApiListeners() {
    ...
    onDataUpdated(async ({ type }) => {
        ...
        if (type === 'punchRecords') {
            ...
            shouldRefreshReport = true;
            shouldRefreshRoster = true;
        } else if (type === 'automationLog') {
            renderAutomationLogs();
        }

        // --- ✨ 魔法修正施展處：安裝緊急通報魔法鈴！ ✨ ---
        if (shouldRefreshReport && ui.modals.reportView.classList.contains('show')) {
        ...
        if (shouldRefreshRoster && ui.modals.roster.classList.contains('show')) {
