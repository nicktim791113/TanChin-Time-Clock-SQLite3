...
import { onPlaySound, onBellHistoryUpdated, onDataUpdated, importThemeImage } from './api.js'; // ✨ 魔法新增
...
import { 
    ...
    handleAutomationTaskAction, resetAutomationTaskForm 
...
function setupModalButtons() {
    ...
    const autoTaskFreqSelect = document.getElementById('auto-task-frequency');
    if (autoTaskFreqSelect) {
        autoTaskFreqSelect.addEventListener('change', (e) => {
            const weeklyContainer = document.getElementById('auto-task-day-weekly-container');
            const monthlyContainer = document.getElementById('auto-task-day-monthly-container');
            const timeContainer = document.getElementById('auto-task-time-container');

            const value = e.target.value;
            weeklyContainer.classList.toggle('hidden', value !== 'weekly');
            monthlyContainer.classList.toggle('hidden', value !== 'monthly');
            timeContainer.classList.toggle('hidden', value === 'immediate');
        });
    }
...
function setupDynamicModalListeners() {
    ...
    // ✨ 新增魔法：為主題編輯器的圖片按鈕添加契約 ✨
    const themeEditorModal = ui.modals.themeEditor;
    themeEditorModal.querySelector('#theme-editor-select-bg-image-btn').addEventListener('click', async () => {
        const result = await importThemeImage();
        if (result.success) {
            const btn = themeEditorModal.querySelector('#theme-editor-select-bg-image-btn');
            btn.textContent = result.name;
            btn.dataset.path = result.path;
            updateThemeEditorPreview();
        }
    });
    themeEditorModal.querySelector('#theme-editor-clear-bg-image-btn').addEventListener('click', () => {
        const btn = themeEditorModal.querySelector('#theme-editor-select-bg-image-btn');
