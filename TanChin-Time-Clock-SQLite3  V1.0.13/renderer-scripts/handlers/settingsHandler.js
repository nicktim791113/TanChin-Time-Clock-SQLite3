...
export function openGreetingsModal(type) {
    ...
    const greetingsIn = (state.greetings && Array.isArray(state.greetings.in)) ? state.greetings.in : [];
    const greetingsOut = (state.greetings && Array.isArray(state.greetings.out)) ? state.greetings.out : [];

    if (type === 'in') {
        title.textContent = '編輯上班問候語';
        textarea.value = greetingsIn.join('\n');
    } else {
        title.textContent = '編輯下班問候語';
        textarea.value = greetingsOut.join('\n');
    }
...
export async function saveGreetings() {
    ...
    const updatedGreetings = {
      in: Array.isArray(state.greetings?.in) ? [...state.greetings.in] : [],
      out: Array.isArray(state.greetings?.out) ? [...state.greetings.out] : [],
    };
