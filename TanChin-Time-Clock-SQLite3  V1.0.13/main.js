...
// --- 魔法修正處：處理立即執行的任務，並返回需要更新的資料類型 ---
ipcMain.handle('execute-task-now', async (event, task) => {
    try {
        const result = await executeAutomationTask(task);
        // 返回成功狀態以及需要通知渲染行程更新的資料類型
        return { success: true, notificationType: result.notificationType };
    } catch (error) {
...
async function executeAutomationTask(task) {
    ...
    let notificationType = null; // ✨ 魔法修正：用於返回給渲染行程的通知類型
    try {
        let message = '';
        let status = 'info';
        let result;
        ...
        switch (task.target) {
            case 'last_week_records':
                ...
                descriptiveDateRange = `上週(${startTime.toISOString().slice(0,10)}至${endTime.toISOString().slice(0,10)})`;
                break;
            case 'last_month_records':
                ...
                startTime.setHours(0, 0, 0, 0);
                endTime.setHours(23, 59, 59, 999);
                descriptiveDateRange = `上月(${startTime.toISOString().slice(0,7)})`;
                break;
        ...
        if (task.task_type === 'export') {
            ...
            if (task.target === 'all_employees') {
                ...
                if (recordsToExport.length === 0) {
                    ...
                    csvContent = headers.join(',') + '\r\n' + rows.join('\r\n');
                    fileName = `考勤報表_${descriptiveDateRange}_(匯出於${todayString}).csv`;
                    const filePath = path.join(desktopPath, fileName);
                    fs.writeFileSync(filePath, Buffer.concat([bom, Buffer.from(csvContent)]));
                    message = `任務成功：已將 [${descriptiveDateRange}] 的 ${recordsToExport.length} 筆考勤紀錄匯出至桌面。`;
                    status = 'success';
                }
            }
